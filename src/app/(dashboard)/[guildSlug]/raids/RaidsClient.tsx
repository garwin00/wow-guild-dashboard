  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="wow-heading text-3xl font-bold" style={{ color: "var(--wow-gold-bright)" }}>Raids</h1>
        {isOfficer && (
      ...
      {showCreate && (
        <form onSubmit={createEvent} style={{ background: "var(--wow-surface)", border: "1px solid rgba(var(--wow-primary-rgb),0.15)", borderRadius: "0.5rem", padding: "1.5rem", marginBottom: "1.5rem" }} className="space-y-4">
          <h2 style={{ color: "var(--wow-text)", fontWeight: 600, marginBottom: "0.5rem" }}>New Raid Event</h2>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs mb-1" style={{ fontFamily: "inherit", color: "var(--wow-text-faint)", letterSpacing: "0.05em", textTransform: "uppercase" }}>Title</label>
            ...
            <div>
              <label className="block text-xs mb-1" style={{ fontFamily: "inherit", color: "var(--wow-text-faint)", letterSpacing: "0.05em", textTransform: "uppercase" }}>Raid Zone</label>
              <input required value={form.raidZone} onChange={(e) => setForm({ ...form, raidZone: e.target.value })}
                placeholder="e.g. Nerub-ar Palace"
                style={{ background: "var(--wow-surface)", border: "1px solid rgba(var(--wow-primary-rgb),0.2)", color: "var(--wow-text)", borderRadius: "0.5rem", padding: "0.5rem 0.75rem", fontSize: "0.875rem", width: "100%", outline: "none" }} />
            </div>
            <div>
              <label className="block text-xs mb-1" style={{ fontFamily: "inherit", color: "var(--wow-text-faint)", letterSpacing: "0.05em", textTransform: "uppercase" }}>Date &amp; Time</label>
          ...
          <div>
            <label className="block text-xs mb-1" style={{ fontFamily: "inherit", color: "var(--wow-text-faint)", letterSpacing: "0.05em", textTransform: "uppercase" }}>Description (optional)</label>
      ...
      {/* Upcoming */}
      <h2 style={{ color: "var(--wow-text-faint)", fontSize: "0.75rem", textTransform: "uppercase", letterSpacing: "0.1em", fontFamily: "inherit", marginBottom: "0.75rem" }}>Upcoming</h2>
      {upcoming.length === 0 ? (
        <p style={{ color: "var(--wow-text-faint)", fontSize: "0.875rem", marginBottom: "1.5rem" }}>{isOfficer ? "No raids scheduled. Create one above." : "No upcoming raids."}</p>
      ) : (
      ...
      {past.length > 0 && (
        <>
          <h2 style={{ color: "var(--wow-text-faint)", fontSize: "0.75rem", textTransform: "uppercase", letterSpacing: "0.1em", fontFamily: "inherit", marginBottom: "0.75rem" }}>Past</h2>
...
function EventCard({ event, guildSlug }: { event: RaidEvent; guildSlug: string }) {
  ...
  return (
    <Link href={`/${guildSlug}/raids/${event.id}`}
      ...
      style={{ background: "var(--wow-surface)", border: "1px solid rgba(var(--wow-primary-rgb),0.15)", borderRadius: "0.75rem" }}
      ...
      <div className="flex items-start justify-between">
        <div>
          <p style={{ color: "var(--wow-text)", fontWeight: 600 }}>{event.title}</p>
          <p style={{ color: "var(--wow-text-muted)", fontSize: "0.875rem", marginTop: "0.125rem" }}>{event.raidZone} · {date.toLocaleDateString("en-GB", { weekday: "short", day: "numeric", month: "short", year: "numeric" })} at {date.toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })}</p>
        </div>
        <span className="text-xs rounded-full px-2 py-0.5" style={statusStyle[event.status]}>{event.status}</span>
      </div>
      <div className="mt-3 flex items-center gap-3">
        <div className="flex-1 rounded-full h-1.5" style={{ background: "var(--wow-bg)" }}>
          <div className="h-1.5 rounded-full" style={{ width: `${Math.min(fill, 100)}%`, background: "var(--wow-gold)" }} />
        </div>
        <span style={{ fontSize: "0.75rem", color: "var(--wow-text-muted)" }}>{event._count.signups}/{event.maxAttendees}</span>
      </div>
    </Link>
  );
}
